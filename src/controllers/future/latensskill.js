    ...
    await LatensSkills.find({})
        .then((r) => {
            if (r.length === 0) {
                return res
                    .status(200)
                    .json({ message: "LatensSkills is empty." });
            };
...
exports.create = async (req, res) => {
    ...
    await d.save()
        .then((r) => {
            return res
                .status(200)
                .json({ success: "Create Latens Skill success", message: r });
        }).catch((err) => {
            return res
                .status(500)
...
exports.update = async (req, res) => {
    ...
    await LatensSkills.findOneAndUpdate(id, update, { new: true })
        .then((r) => {
            return res
                .status(200)
                .json({ success: "Update LatensSkill success", message: r });
        }).catch((err) => {
            return res
                .status(500)
                .json({ error: err.message });
...
exports.remove = async (req, res) => {
    ...
    await LatensSkills.findOneAndDelete({ _id: id })
        .then((r) => {
            return res
                .status(200)
                .json({ message: "Delete LatensSkill success." });
        }).catch((err) => {
            return res
                .status(500)
